...
export default function ReportCard({ report, className = "" }: ReportCardProps) {
  const imageSrc = report.hero_image_url ?? "/images/fc-imabari-community.jpg"
    ...
    <Link href={`/reports/${report.slug}`}>
      ...
      >
        <div className="relative h-40 w-full">
          <Image src={imageSrc} alt={report.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
        </div>
